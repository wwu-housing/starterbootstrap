...
class admin_plugin_config extends DokuWiki_Admin_Plugin {
    ...
    function html() {
      ...
      if ($this->_config->locked)
        ptln('<div class="alert alert-info">'.$this->getLang('locked').'</div>');
      elseif ($this->_error)
        ptln('<div class="alert alert-error">'.$this->getLang('error').'</div>');
      elseif ($this->_changed)
        ptln('<div class="alert alert-success">'.$this->getLang('updated').'</div>');
      ...
      foreach($this->_config->setting as $setting) {
        if (is_a($setting, 'setting_hidden')) {
          ...
          ptln('  <table class="table table-hover table-bordered">');
        } else {
          // config settings
          list($label,$input) = $setting->html($this, $this->_error);

          $class = $setting->is_default() ? ' class="default info"' : ($setting->is_protected() ? ' class="protected error"' : '');
          $error = $setting->error() ? ' class="error"' : ' class=""';
      ...
      if (!$this->_config->locked) {
        ...
        ptln('  <input type="submit" name="submit" class="btn btn-primary" value="'.$lang['btn_save'].'" accesskey="s" />');
